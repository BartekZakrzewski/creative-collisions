  ...
  <div className="bg-white shadow-lg rounded-lg p-0 lg:p-8 pb-12 mb-8">
    ...
    <div className="block lg:flex text-center items-center justify-center mb-8 w-full">
      <div className="flex flex-col-reverse items-center justify-center mb-4 lg:mb-0 w-full lg:w-auto mr-8 items-center">
        {post.author.map((auth, index) => (
          <Link href={`/authors/${auth.slug}`}>
            <div key={index} className="cursor-pointer flex items-center justify-center mb-2">
              <Image
                unoptimized
                loader={grpahCMSImageLoader}
                alt={auth.name}
      ...
      <div className="font-medium text-gray-700">
        ...
        <span className="align-middle">{moment(post.publishingDate).format('DD MMM, YYYY')}</span>
      </div>
    </div>
    ...
    <div className="text-center">
      <Link href={`/post/${post.slug}`}>
        <span className="transition duration-500 ease transform hover:-translate-y-1 inline-block bg-purple-500 text-lg font-medium rounded-full text-white px-8 py-3 cursor-pointer">Continue Reading</span>
      </Link>
    </div>
  </div>
);
